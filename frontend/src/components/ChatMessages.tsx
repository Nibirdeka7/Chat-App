  ...
  return (
    <div className='flex-1 overflow-hidden'>
        <div className="h-full max-h-[calc(100vh-215px)] overflow-y-auto p-2 space-y-2 custom-scroll">
            {
                !selectedUser? <p className="text-gray-900 text-center mt-20">Select a user to start chatting 💭</p> :
                 <>
                 {
                    uniqueMessages.map((e,i) => {
                        ...
                        return (
                            <div className={`flex flex-col gap-1 mt-2 ${isSentByMe ? "items-end" : "items-start"}`}
                                key={uniqueKey}
                            >
                                <div className={`rounded-lg p-3 max-w-sm ${isSentByMe ? "bg-gray-800 text-white border border-black" : "bg-white border border-black text-black"} `}>
                                    {
                                ...
                                <div className={`flex items-center gap-1 txt-xs text-gray-700 ${isSentByMe ? "pr-2 flex-row-reverse" : "pl-2"}`}>
                                    
                                        <span>{moment(e.createdAt).format("hh:mm A . MMM D")}</span>
                                        {
                                            isSentByMe && <div className="flex items-center ml-1">
                                               {
                                                e.seen ? (<div className="flex items-center gap-1 text-green-600">
                                                    <CheckCheck className='w-3 h-3'/>
                                                    {
                                                        e.seenAt && <span>{moment(e.seenAt).format("hh:mm A")}</span>
                                                    }
                                                </div>) : (<Check className='w-3 h-3 text-gray-900'/>
                                               )}
                                            </div>
                                        }
